...
im.ggplot <- function(input_raster, layerfill = 1) {
  ...
  layerfill <- colnames(df)[layerfill + 2]  # Adjust for x, y columns

  print(
    ggplot(df, aes(x = x, y = y, fill = !!sym(layerfill))) +
